...
function getUserByMilitaryOrSecretCode(value) {
  ...
  return state.cia_users.find((u) => u.publicCode === wanted || u.secretCode === wanted) || null;
}
...
function chatKey(a, b) {
  return [clean(a, 100), clean(b, 100)].sort().join('::');
}
...
io.on('connection', (socket) => {
  ...
  socket.on('account:create', (payload, cb) => {
    try {
      ...
      if (state.cia_accounts.some((a) => lower(a.pendingPrimary?.name) === lower(characterName))) {
        return no(cb, 'يوجد طلب معلق لهذه الشخصية.');
      }
      ...
      return ok(cb, {
        ...
        pending: true,
        message: 'تم إنشاء الحساب. استخدم 0012 من شاشة الدخول لإرسال طلب القبول.'
      });
    } catch (error) {
  ...
  socket.on('character:request', (payload, cb) => {
    ...
    if (isChief(user)) {
      ...
      saveState();
      const result = ok(cb, { direct: true, user: publicUser(newUser, user), secretCode: newUser.secretCode });
    ...
    addAuditLog('طلب شخصية', user, null, `طلب ${user.name} إنشاء شخصية ${name}.`);
    saveState();

    socket.emit('character:request:result', ok(null, { request: queueItem }));
    for (const currentSocket of io.sockets.sockets.values()) {
      const currentUser = currentSocket.userId ? getUserById(currentSocket.userId) : null;
  ...
  socket.on('admin:changeChiefSecret', (payload, cb) => {
    const actor = requireSocketUser(socket);
    if (!actor || !isChief(actor)) return no(cb, 'هذه الصلاحية متاحة لـ CIA CHIEF فقط.');
    const newSecret = clean(payload?.newSecretCode, 100);
  ...
  socket.on('bank:setRankSalary', (payload, cb) => {
    ...
    addAuditLog('تعديل راتب رتبة', actor, null, `تم تحديد راتب رتبة ${rankLabel(rank)} إلى ${amount}. تم تحديث ${updated} شخصية وحفظ الإعداد للمنضمين الجدد.`);
    saveState();
    const result = ok(cb, { rank, label: rankLabel(rank), amount, updated });
  ...
  socket.on('bank:paySalary', (payload, cb) => {
    ...
    if (paid === null) return no(cb, 'لا يمكن الصرف الآن. يجب أن تكون الشخصية مسجلة دخول ولم تستلم راتبها خلال آخر 24 ساعة، والحساب غير مجمد/موقوف.');
  ...
  socket.on('operation:create', (payload, cb) => {
    try {
      const actor = requireSocketUser(socket);
      if (!actor || (!isChief(actor) && !isSenior(actor) && !isHighCommander(actor))) return no(cb, 'إنشاء المهام متاح لـ CIA CHIEF وSenior Commander CIA وHigh Commander.');
      ...
      for (const code of memberCodes) {
        const member = getUserByPublicCode(code);
        if (!member || member.suspended || member.activeService === false) return no(cb, `كود العضو ${code} غير صالح.`);
      }
      ...
      addAuditLog('إنشاء عملية ميدانية', actor, commander, `تم إنشاء ${title} — ${risk}.`);
      saveState();
      const result = ok(cb, { operation: operationSanitize(op, actor) });
      socket.emit('operation:create:result', result);
      for (const code of memberCodes) {
        const target = getUserByPublicCode(code);
        const set = target ? sessions.get(target.id) : null;
        if (set) for (const sid of set) {
          const s = io.sockets.sockets.get(sid);
          if (s) s.emit('operation:assigned', { ok: true, operation: operationSanitize(op, target) });
        }
      }
      emitOperationState();
      emitState();
  ...
  socket.on('operation:updateStatus', (payload, cb) => {
    ...
    if (!(isChief(actor) || isSenior(actor) || isHighCommander(actor) || op.commanderCode === actor.publicCode || op.memberCodes.includes(actor.publicCode))) return no(cb, 'لا تملك صلاحية تحديث العملية.');
    op.status = status; op.updatedAt = now();
    addAuditLog('تحديث عملية', actor, null, `${op.id}: ${status}`);
    saveState();
    const result = ok(cb, { operation: operationSanitize(op, actor) });
  ...
  socket.on('operation:addNote', (payload, cb) => {
    ...
    if (!(isChief(actor) || isSenior(actor) || isHighCommander(actor) || op.commanderCode === actor.publicCode || op.memberCodes.includes(actor.publicCode))) return no(cb, 'لا تملك صلاحية إضافة ملاحظة.');
    op.notes.push({ at: now(), authorCode: actor.publicCode, authorName: actor.name, text });
    op.notes = op.notes.slice(-100); op.updatedAt = now();
    saveState();
    const result = ok(cb, { operation: operationSanitize(op, actor) });
  ...
  socket.on('operation:updateCrew', (payload, cb) => {
    const actor = requireSocketUser(socket);
    if (!actor || (!isChief(actor) && !isSenior(actor) && !isHighCommander(actor))) return no(cb, 'إدارة الطاقم متاحة للقيادة فقط.');
    const op = state.cia_operations.find((x) => x.id === clean(payload?.operationId, 120));
    if (!op) return no(cb, 'العملية غير موجودة.');
    const codes = [...new Set((Array.isArray(payload?.memberCodes) ? payload.memberCodes : []).map((x) => clean(x,100)).filter(Boolean))];
    for (const code of codes) { const u = getUserByPublicCode(code); if (!u || u.suspended || u.activeService === false) return no(cb, `كود العضو ${code} غير صالح.`); }
    op.memberCodes = codes; op.updatedAt = now();
    addAuditLog('تعديل طاقم عملية', actor, null, `تم تعديل طاقم ${op.id}.`);
    saveState();
    const result = ok(cb, { operation: operationSanitize(op, actor) });
  ...
  socket.on('operation:delete', (payload, cb) => {
    ...
    const [removed] = state.cia_operations.splice(i, 1);
    addAuditLog('حذف عملية ميدانية', actor, null, `تم حذف ${removed.title}.`);
    saveState();
  ...
  socket.on('radio:ptt:start', (payload, cb) => {
    ...
    if (!user) return no(cb, 'يجب تسجيل الدخول أولاً.');
    const channel = clean(payload?.channel || user.radioChannel || 'CH-1', 50);
    const data = { channel, userCode: user.publicCode, userName: '', speaking: true };
    for (const sid of radioTargets(channel)) {
      const targetSocket = io.sockets.sockets.get(sid);
  ...
  socket.on('radio:ptt:stop', (payload, cb) => {
    ...
    if (!user) return no(cb, 'يجب تسجيل الدخول أولاً.');
    const channel = clean(payload?.channel || user.radioChannel || 'CH-1', 50);
    const data = { channel, userCode: user.publicCode, userName: '', speaking: false };
    for (const sid of radioTargets(channel)) {
      const targetSocket = io.sockets.sockets.get(sid);
      if (targetSocket) targetSocket.emit('radio:state', data);
    }
  ...
  socket.on('sos:broadcast', (payload, cb) => {
    ...
    let message = clean(payload?.text, 3000);
    /* Force public military identity only. Remove any accidental real name. */
    message = message
      .replace(user.name, `#${user.publicCode}`)
      .replace(new RegExp(user.name.replace(/[.*+?^${}()|[\\]\\]/g, '\\$&'), 'gi'), `#${user.publicCode}`);
    ...
    addAuditLog('إرسال S.O.S', user, null, `بلاغ طوارئ من #${user.publicCode}.`);
    saveState();
...
process.on('uncaughtException', (error) => {
  console.error('[BLACK RIDGE] Uncaught exception:', error);
});

process.on('unhandledRejection', (reason) => {
  console.error('[BLACK RIDGE] Unhandled rejection:', reason);
});
