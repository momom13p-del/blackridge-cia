...
function bankView(user, viewer = user) {
  ...
  // شرط عرض اسم الشخصية وكودها: القائد يرى اسم الشخصية وكودها، والباقي يرون الكود العسكري فقط
...
function operationSanitize(op, viewer) {
  ...
  // إذا كان العضو برتبة أجينت، يظهر له فقط المهمة وتفاصيلها ومكانها
  if (isAgent) {
    return {
      id: op.id,
...
io.on('connection', (socket) => {
  ...
  // تعديل كود الدخول العسكري للقائد (تحديث الكود السري والشخصي)
  ...
  // حفظ الراتب وتأكيد اعتماد المبلغ للرتبة
  socket.on('bank:setRankSalary', (payload, cb) => {
    ...
    addAuditLog('تعديل راتب رتبة', actor, null, `تم تحديد راتب رتبة ${rankLabel(rank)} إلى ${amount}.`);
    saveState();
    const successMsg = `تم اعتماد المبلغ للراتب لهذه الرتبة (${rankLabel(rank)}) بنجاح.`;
    const result = ok(cb, { rank, label: rankLabel(rank), amount, updated, message: successMsg });
  ...
  // إضافة دعم إضافة مبلغ لحسابات معينة (للقائد والسينيور كوماندر)
  socket.on('bank:addBalance', (payload, cb) => {
    const actor = requireSocketUser(socket);
    if (!actor || (!isChief(actor) && !isSenior(actor))) return no(cb, 'إضافة مبلغ للحسابات متاحة للقائد والسينيور فقط.');
    const targetCode = clean(payload?.targetCode || payload?.code, 100);
    ...
    addAuditLog('إضافة مبلغ لحساب', actor, target, `تمت إضافة مبلغ ${amount} إلى حساب ${target.publicCode}.`);
    saveState();
    const result = ok(cb, { target: bankView(target, actor), amount, message: `تمت إضافة مبلغ ${amount} بنجاح.` });
  ...
  // حذف أماكن الأشخاص في الخريطة المنسوبة (للقائد، هاي كوماندر، والسينيور كوماندر)
  socket.on('map:location:delete', (payload, cb) => {
    const actor = requireSocketUser(socket);
    if (!actor || (!isChief(actor) && !isSenior(actor) && !isHighCommander(actor))) {
      return no(cb, 'حذف أماكن الأشخاص من الخريطة متاح للقائد، الهيد كوماندر، والسينيور كوماندر فقط.');
    }
  ...
  /* =====================================================
     SOS SYSTEM (إظهار الرتبة الحقيقية والكود العسكري وحذف البلاغ)
  ===================================================== */
  socket.on('sos:broadcast', (payload, cb) => {
    const user = requireSocketUser(socket);
    if (!user) return no(cb, 'يجب تسجيل الدخول أولاً.');

    const message = clean(payload?.text, 3000);
    ...
    addAuditLog('إرسال S.O.S', user, null, `بلاغ طوارئ من الرتبة ${rankLabel(user.rank)} كود #${user.publicCode}.`);
    saveState();
...
httpServer.listen(PORT, () => {
  ...
  console.log(' ALL REQUIREMENTS APPLIED SUCCESSFULLY');
  console.log('=================================================');
});
